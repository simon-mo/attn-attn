...
parser.add_argument('--base-net', type=str,
                    help='inception_v3, base network')
...
file_name = time + '_' + '_'.join(list(map(str, [train_phase, base_net, attn_loss, attn_shape, 'inception']))) + '.log'
...
def get_loader(dirname, batch_size=32):
    trainset = torchvision.datasets.ImageFolder(root=f'../inception_data/{dirname}', transform=transform)
...
trainset = torchvision.datasets.ImageFolder(root=f'../inception_data/train', transform=transform)
...
def train_one(use_attn=use_attn):
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        if layer == 'bn':
            ...
            loss1 = cls_criterion(output, labels)
            loss2 = cls_criterion(aux_output, labels)
