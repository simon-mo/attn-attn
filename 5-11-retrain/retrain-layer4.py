...
file_name = time + '_' + '_'.join(list(map(str, [train_phase, base_net, attn_loss, attn_shape, 'layer4_fc']))) + '.log'
...
import re
def turn_off_grad_for(network_name, regex):
    formatted_keys = get_formatted_keys(network_name)
    for k in formatted_keys:
        obj = eval(f'{network_name}.' + k)
        if re.match(regex, k):
...
for a, l in enumerate(train_phase):
    ...
    #bn is fine b/c it won't change anything
    if layer == 'fc':
        turn_off_grad_except('resnet_attn', ['layer4', 'fc'])
        turn_off_grad_for('resnet_attn', 'resnet_attn.layer4.*attn_weights')
        turn_off_grad_for('resnet_attn', 'resnet_attn.layer4.*running.*') # just in case
        resnet_attn.eval()
        resnet_attn.layer4.train()
    elif layer == 'attn_weights':
        turn_off_grad_for('resnet_attn', 'resnet_attn.layer4.*attn_weights')        
